...
class RowIterator implements IteratorInterface
{
	...
	/** @var bool Indicates whether pointer there is tolerance position */
	protected $tolerance = false;
	
	/** @var int number of row that had been parsed */
	protected $counter = 0;
     ...
     */
    public function __construct($filePath, $sheetDataXMLFilePath, $sharedStringsHelper, $startRowFromSheet)
    {
        $this->filePath = $filePath;
        $this->sheetDataXMLFilePath = $this->normalizeSheetDataXMLFilePath($sheetDataXMLFilePath);

		$this->numReadRows = $startRowFromSheet;
        ...
        try {
            while ($this->xmlReader->read()) 
			{
				...
				else if ($this->xmlReader->isPositionedOnStartingNode(self::XML_NODE_ROW)) //
				{
				    ...
				    if($this->counter >= $this->numReadRows)//$this->xmlReader->moveToAttributeNo($this->numReadRows);$this->counter >= $this->numReadRows)
					{
				...
				else if ($this->xmlReader->isPositionedOnStartingNode(self::XML_NODE_CELL) && $this->tolerance) 
				{
				...
				else if ($this->xmlReader->isPositionedOnEndingNode(self::XML_NODE_ROW) && $this->tolerance) 
				{
				...
				else if ($this->xmlReader->isPositionedOnEndingNode(self::XML_NODE_WORKSHEET) && $this->tolerance) 
				{
